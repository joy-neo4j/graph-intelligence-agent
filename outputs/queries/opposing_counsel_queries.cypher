...
// Q1. Which law firms have acted for sovereign states resisting enforcement of arbitral awards, and have any also appeared on the claimant side?
// Traversal: LawFirm → Judgment → Party (sovereign defendant) + cross-check HAS_CLAIMANT_FIRM for same firm

MATCH (j:Judgment)-[:HAS_DEFENDANT]->(p:Party)
WHERE
  toLower(p.name) CONTAINS 'republic' OR
  toLower(p.name) CONTAINS 'federation' OR
  toLower(p.name) CONTAINS 'kingdom' OR
  p.partyType CONTAINS 'sovereign'
...
RETURN
  firm,
  size(defendantCases) AS defended,
  collect(DISTINCT j2.neutralCitation) AS alsoClaimant
ORDER BY defended DESC;

// ────────────────────────────────────────────────────────────

// Q2. Who appeared for the Russian Federation across its cases before the Commercial Court, and what was the outcome?
...
// Q3. For a state client challenging an award on jurisdictional grounds — which barristers have the most experience on that defence side? (requires CSV-enriched chambers + kcYear)
...
// Q4. Has Quinn Emanuel ever acted against a sovereign state respondent, or do they consistently act for one side?
...
RETURN
  ...
  count(
    CASE
      WHEN EXISTS { (j)-[:HAS_CLAIMANT_COUNSEL]->(c) } THEN 1
    END) AS claimantApps,
  count(
    CASE
      WHEN EXISTS { (j)-[:HAS_DEFENDANT_COUNSEL]->(c) } THEN 1
    END) AS defendantApps
...
MATCH
  (j:Judgment)-[:CONCERNS]->
  (t:LegalTopic {name: 'investment treaty arbitration'})
MATCH (j)-[:HAS_CLAIMANT_FIRM]->(cf:LawFirm)
MATCH (j)-[:HAS_DEFENDANT_FIRM]->(df:LawFirm)
RETURN
  j.neutralCitation,
  j.outcome,
  cf.name AS claimantFirm,
  cf.arbitrationTier AS claimantTier,
  df.name AS defendantFirm,
  df.arbitrationTier AS defendantTier;
...
MATCH (j:Judgment)-[:HAS_DEFENDANT]->(p:Party)
WHERE
  toLower(p.name) CONTAINS 'republic' OR toLower(p.name) CONTAINS 'federation'
MATCH (j)-[:HAS_DEFENDANT_COUNSEL]->(c:Counsel)
...
MATCH (n:Judgment)
RETURN 'Judgment' AS label, count(n) AS count
  UNION ALL
MATCH (n:Counsel)
RETURN 'Counsel' AS label, count(n) AS count
  UNION ALL
MATCH (n:LawFirm)
RETURN 'LawFirm' AS label, count(n) AS count
  UNION ALL
MATCH (n:Party)
RETURN 'Party' AS label, count(n) AS count
  UNION ALL
MATCH (n:LegalTopic)
RETURN 'LegalTopic' AS label, count(n) AS count
  UNION ALL
MATCH (n:Legislation)
RETURN 'Legislation' AS label, count(n) AS count
  UNION ALL
MATCH (n:Judge)
RETURN 'Judge' AS label, count(n) AS count
  UNION ALL
MATCH (n:Document)
RETURN 'Document' AS label, count(n) AS count
  UNION ALL
MATCH (n:Chunk)
RETURN 'Chunk' AS label, count(n) AS count;

// ────────────────────────────────────────────────────────────

// Relationship coverage — key traversals for target questions
MATCH ()-[r:HAS_CLAIMANT_COUNSEL]->()
RETURN 'HAS_CLAIMANT_COUNSEL' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:HAS_DEFENDANT_COUNSEL]->()
RETURN 'HAS_DEFENDANT_COUNSEL' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:HAS_CLAIMANT_FIRM]->()
RETURN 'HAS_CLAIMANT_FIRM' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:HAS_DEFENDANT_FIRM]->()
RETURN 'HAS_DEFENDANT_FIRM' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:PRESIDED_BY]->()
RETURN 'PRESIDED_BY' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:CONCERNS]->()
RETURN 'CONCERNS' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:CITES]->()
RETURN 'CITES' AS rel, count(r) AS count
  UNION ALL
MATCH ()-[r:EXTRACTED_FROM]->()
RETURN 'EXTRACTED_FROM' AS rel, count(r) AS count;
...
MATCH (c:Counsel)
WHERE NOT (c)--()
RETURN 'Orphan Counsel' AS issue, count(c) AS count
  UNION ALL
MATCH (f:LawFirm)
WHERE NOT (f)--()
RETURN 'Orphan LawFirm' AS issue, count(f) AS count
  UNION ALL
MATCH (t:LegalTopic)
WHERE NOT (t)--()
RETURN 'Orphan LegalTopic' AS issue, count(t) AS count
  UNION ALL
MATCH (l:Legislation)
WHERE NOT (l)--()
RETURN 'Orphan Legislation' AS issue, count(l) AS count;
...
WHERE cnt > 1
RETURN 'Duplicate Counsel' AS issue, name, cnt
ORDER BY cnt DESC
LIMIT 10;

MATCH (f:LawFirm)
WITH f.name AS name, count(*) AS cnt
WHERE cnt > 1
RETURN 'Duplicate LawFirm' AS issue, name, cnt
ORDER BY cnt DESC
LIMIT 10;
...
MATCH (c:Counsel)
WHERE c.chambers IS NOT NULL
RETURN 'Counsel with chambers' AS metric, count(c) AS count
  UNION ALL
MATCH (c:Counsel)
WHERE c.chambers IS NULL
RETURN 'Counsel without chambers' AS metric, count(c) AS count
  UNION ALL
MATCH (f:LawFirm)
WHERE f.firmType IS NOT NULL
RETURN 'LawFirm with firmType' AS metric, count(f) AS count
  UNION ALL
MATCH (l:Legislation)
WHERE l.url IS NOT NULL
RETURN 'Legislation with URL' AS metric, count(l) AS count;
...
RETURN 'Embedded chunks' AS metric, count(c) AS count
  UNION ALL
MATCH (c:Chunk)
WHERE c.text_embedding IS NULL
RETURN 'Unembedded chunks' AS metric, count(c) AS count;