...
<!-- Windows note: this file is a real copy because Git symlinks don't resolve on Google Drive's FUSE layer.
     Canonical source: .agents/skills/develop-neo4j-graph/SKILL.md
     Reference files:  .agents/skills/develop-neo4j-graph/references/
     Keep in sync with the canonical source when updating. -->
...
**CSV data:** Read samples from `data/csv/`. Reference [HANDLE_STRUCTURED_DATA.md](.agents/skills/develop-neo4j-graph/references/HANDLE_STRUCTURED_DATA.md) for discovery guidance, including the cross-file ID consistency check.

**PDF data:** List files in `data/pdf/` using Shell: `find data/pdf -name "*.pdf" | sort`. (Use Shell rather than file-picker/glob tools — `data/pdf/` is gitignored so those tools may not see PDFs.) For each PDF, use the Read tool with `pages: "1-2"` to sample content; if the Read tool is blocked by gitignore, fall back to Shell: `uv run --with pymupdf python3 -c "import fitz; doc=fitz.open('data/pdf/FILENAME.pdf'); print(doc[0].get_text()[:2000])"`. Reference the Parse Mode Guide in [HANDLE_UNSTRUCTURED_DATA.md](.agents/skills/develop-neo4j-graph/references/HANDLE_UNSTRUCTURED_DATA.md) to identify the appropriate parse mode for each document type.
...
Reference [GRAPH_DATA_MODELING_MCP.md](.agents/skills/develop-neo4j-graph/references/GRAPH_DATA_MODELING_MCP.md) for the full modeling process using the `neo4j-data-modeling` MCP server.
...
Reference [HANDLE_STRUCTURED_DATA.md](.agents/skills/develop-neo4j-graph/references/HANDLE_STRUCTURED_DATA.md) for ingestion details.
...
- **pymupdf** → [PYMUPDF_MODE.md](.agents/skills/develop-neo4j-graph/references/PYMUPDF_MODE.md)
- **docling** → [DOCLING_MODE.md](.agents/skills/develop-neo4j-graph/references/DOCLING_MODE.md)
- **page_image** → [PAGE_IMAGE_MODE.md](.agents/skills/develop-neo4j-graph/references/PAGE_IMAGE_MODE.md)
- **vlm_blocks** → [VLM_BLOCKS_MODE.md](.agents/skills/develop-neo4j-graph/references/VLM_BLOCKS_MODE.md)
...
If CSV data is also present, run entity reconciliation to check for name mismatches between extracted and structured nodes. See [HANDLE_UNSTRUCTURED_DATA.md](.agents/skills/develop-neo4j-graph/references/HANDLE_UNSTRUCTURED_DATA.md).
...
**[CHATBOT mode]** → Load [CHATBOT_MODE.md](.agents/skills/develop-neo4j-graph/references/CHATBOT_MODE.md):
...
**[ANALYTICAL mode]** → Load [ANALYTICAL_MODE.md](.agents/skills/develop-neo4j-graph/references/ANALYTICAL_MODE.md):
